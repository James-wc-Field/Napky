...
export async function getOpenGraphTags(url: string) {
  ...
  const attributes = html
    ...
    .reduce<{ [property: string]: string }>((accumulator, element) => {
      if (
        element.getAttribute("property")?.startsWith("og") &&
        element.hasAttribute("content")
      ) {
        accumulator[element.getAttribute("property")!] =
...
export async function generateSummary(url: string, apiKey: string) {
  ...
  const completion = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: "user",
        content: `Your task is to summarize the website content using the provided html text. Your output should use the following template:
    ####Summary
